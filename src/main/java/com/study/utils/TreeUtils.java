...
//    }
    ...
    public static TreeNode2 buildTree(String[] nodes) {
        List<TreeNode2> treeNodes = new ArrayList<TreeNode2>();
        for (String data : nodes) {
            treeNodes.add(new TreeNode2(data));
        }
        //创建一个二叉树
        //创建其他节点  每个节点的左子节点都是这个节点的2倍+1, 右子节点都是这个节点的2倍+2
        for (int i = 0; i < nodes.length / 2; i++) {
    ...
    private static int getTreeDepth(TreeNode2 root) {
        return root == null ? 0 : (1 + Math.max(getTreeDepth(root.left), getTreeDepth(root.right)));
    }
    ...
    private static void writeArray(TreeNode2 currNode, int rowIndex, int columnIndex, String[][] res, int treeDepth) {
        // 保证输入的树不为空
        if (currNode == null) return;
        // 先将当前节点保存到二维数组中
        res[rowIndex][columnIndex] = String.valueOf(currNode.val);
    ...
    public static void show(TreeNode2 root) {
        if (root == null) System.out.println("EMPTY!");
        // 得到树的深度
        int treeDepth = getTreeDepth(root);
